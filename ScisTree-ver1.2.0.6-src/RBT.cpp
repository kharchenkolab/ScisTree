...
//treeOpt.Dump();
			...
			if( tr2.pCurNode->IsLeaf() == true && 
				( ( mapCherry1.size() == 1 && (pCurNode == pLeaf1 || pCurNode == pLeaf2 || tr2.pCurNode == pLeaf1 ||  tr2.pCurNode == pLeaf2 )  )  
				|| (
						mapCherry1.size() == 2 && 
						( ( (pCurNode == pLeaf1 || pCurNode == pLeaf2) && ( tr2.pCurNode == pLeaf3 ||  tr2.pCurNode == pLeaf4 ) )
						||  ( ( pCurNode == pLeaf3 || pCurNode == pLeaf4) && ( tr2.pCurNode == pLeaf1 ||  tr2.pCurNode == pLeaf2 )  )   
						)  
						)   )  
				)
			{
...
//cout << "Here...\n";
    ...
    for(int i=0; i<numTNodes; ++i)
    {
        YW_ASSERT_INFO( (listNodeLeftChild[i] >=0 && listNodeRightChild[i] >=0) 
            || (listNodeLeftChild[i] <0 && listNodeRightChild[i] <0), "WRONG" );
...
//cout << "set right child node " << prcind << " to " << ppos << endl;
    ...
    for(int i=0; i<GetNodesNum()-1; ++i)
    {
        YW_ASSERT_INFO( listParentNodePos[i]>=0 && 
            listEdgeDist[i] >=0, "Some nodes are not correctly set." );
    }
}
